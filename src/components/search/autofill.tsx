    ...
    return (
        <div className="flex items-center justify-center">
            <div tabIndex={1} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} className="relative">
                <input onChange={(e) => (setResultsShown(results.filter((el) => el.toLowerCase().includes(e.target.value.toLowerCase()))))} type="text" className="w-[200px] px-3 py-1 text-lg rounded-full border-2 border-gray-500 focus:border-gray-700 outline-none transition" placeholder="Search for value..."/>
                ...
                {focus && (
                    <div className="absolute mt-1 w-full p-2 bg-white shadow-lg rounded-bl rounded-br max-h-56 overflow-y-auto">
                        {resultsShown.sort().map((item, index) => {
                            return (
                                <div key={index} onMouseDown={() => {callback(item, value)}} className="cursor-pointer hover:bg-black hover:bg-opacity-10 p-2">
                                    {item}
                                </div>
                            );
                        })}
