...
const Header: React.FC = () => {
  ...
  return (
    <div className='bg-[rgb(131,182,94)] h-[50px] flex flex-row px-5 py-2 items-center justify-between font-body font-poppins text-2xl text-white font-thin w-full'>
      ...
      <span className='relative block w-full max-w-[800px] mx-[7%]'>
        <form onSubmit={onSearch}>
          <input 
            value={searchQuery} 
            onChange={event => setSearchQuery(event.target.value)} 
            className='bg-[#b5ceb0] w-full text-black pl-10 pr-5 py-1 text-sm rounded-3xl placeholder:italic placeholder:text-slate-400 focus:outline-none my-1 drop-shadow-lg' 
            placeholder='search...'>
          </input>
        </form>  
      ...
      <div className='flex flex-row gap-2 mr-5'>
        ...
        <button className='flex flex-row gap-2 hover:bg-[rgb(149,201,112)] px-2' onClick={sessionData ? () => void signOut() : () => void router.push(`/auth/login?prev=${router.asPath}`)}>
