...
const Table: React.FC<props> = ({
  ...
  const [filterState, setFilterState] = useState<INormalFilter | undefined>(
    filterNormal
  );

  const test = api. columns.getAll.useQuery();
                  ...
                  <tr className={`bg-gray-200 ${show[index] ? "" : "hidden"}`}>
                    <td colSpan={2} className="bg-gray-200 px-5">
                      <div className="grid grid-cols-2">
                        ...
                        {sample.data.Infectious_Disease_Test_Result !== null &&
                        sample.data.Infectious_Disease_Test_Result !== ""
                          ? sample.data.Infectious_Disease_Test_Result
                          : "NaN"}
                    ...
                    >
                      <div className="grid grid-cols-2">
                        ...
                        sample.data.Diagnosis_Remarks.length > 0
                          ...
                          : "NaN"}
                        <span>ICD:</span>{" "}
                        {sample.data.ICD_Code && sample.data.ICD_Code.length > 0
